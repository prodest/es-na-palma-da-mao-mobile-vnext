...
export class SchedulingService {
  ...
  getUnits(serviceId: number): Observable<ListItem[]> {
    return this.http.get<ListItem[]>(`${this.env.api.scheduling}/servicos/${serviceId}/unidades`);
  }
  ...
  save(info) {
    return this.http.post(`${this.env.api.scheduling}/agendamentos`, info);
  }

  sendEmail(email: string, info: any) {
    return this.http.post(this.env.api.mailer, {
  ...
  getDocumentTypes(): Observable<ListItem[]> {
    return this.http.get<ListItem[]>(`${this.env.api.scheduling}/tiposdocumentos`);
  }

  findScheduling(info: any): any {
    return this.http.get(`${this.env.api.scheduling}/agendamentos`, {params: {
  ...
  cancelScheduling(id: string) {
    return this.http.delete(`${this.env.api.scheduling}/agendamentos/${id}`);
  }
}
